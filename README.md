...
| **모아서 본다** | `모아보기` — 폰은 사진 격자, 노트북(900px↑)은 **봉투가 3D로 떠 있는 편지 공간**. 연도·달 침으로 좁히고, 남긴 말로 찾으면 걸린 봉투만 남습니다 |
...
| 화면 | 무엇을 하나 |
|---|---|
| 목록 `/` | 자란 순서 타임라인. 사진 · 도착한 편지들 · **그 시기의 이름**(`만 7세 2개월`). **남긴 말로 찾기**는 모아보기에서 |
...
| **모아보기** | `/grid` | 폰: 사진 격자 훑기 · 노트북: **편지 공간**(봉투·궤도·미리보기). 연도·달로 좁히고 **남긴 말로 찾기** |
...
| 안 가져온 것 | 왜 |
|---|---|
| **WebGL 캔버스** | CSS 3D로 같은 것을 의존성 0으로 — 배치는 서버가 렌더하고 JS는 궤도·줌뿐 |
...
| **899px** | 격자 3열 | 칸 **272px** |
| **900px 이상** | **편지 공간** — 봉투가 3D로 떠 있고 격자는 `display: none` | 봉투 150×96px · 궤도 ±12°/±16° · 줌 0.55–1.7 |

> 🔴 **v1의 "4열 268px" 실측은 900px 미만에서만 유효해졌습니다.** 넓은 화면의 모아보기가
> 격자에서 편지 공간으로 바뀌었기 때문입니다([16-letters §3](./docs/decisions/16-letters.md)).
> 공간의 봉투 **배치는 서버가 렌더**합니다(id 해시 → 인라인 스타일) — JS가 하는 일은
> 드래그 궤도와 스크롤 줌뿐이라, **JS 없이도 공간·hover 미리보기·침·검색이 다 동작합니다.**
> 폰이 격자를 유지하는 이유: 한 손 30초 사용자에게 궤도는 장난감입니다.

...
**이 표의 화면 전부에 같은 상단 바가 있습니다** — `한 해, 한 권` / `되짚어보기` / `아이 정보`.
404와 실패 화면에도 있어서 **막힌 자리에서 나갈 길이 늘 하나는 있습니다.**
사이드바가 아닌 이유는 주 사용자가 폰 세로 한 손이라 가로 폭을 상시로 내줄 수 없기 때문이고,
`sticky`가 아닌 이유는 상단 자리를 목록의 **해 제목**이 이미 쓰기 때문입니다 — 스크롤 중에
더 필요한 쪽은 *지금 어느 해를 보고 있는지*입니다.

`/orders`는 상단 바에 없습니다. 주문이 0건일 때 주문 목록을 상시로 보여주면
**아직 할 수 없는 일을 화면이 먼저 말하는** 상태가 됩니다. 주문이 생기면 홈의 책 구역 제목이
`한 해가 한 권 · 주문 1건`으로 바뀌면서 링크가 나타납니다.
...
에디토리얼 세리프(웹폰트 2MB), 스크롤 진입 애니메이션(첫 화면이 아이 그림 격자입니다),
...
| 화면 폭 | 목록 격자 | 책 줄 | 상세 |
|---|---|---|---|
| **1440px** | 4열 · 카드 322px | **5열** | 사진 옆에 말 (2단) |
| **375px** | 1열 · 카드 343px | 1열 | 사진 아래 말 (1단) |

목록 격자와 책 줄이 **같은 규칙**(`auto-fill` + `minmax`)을 쓴다. 전에는 책 줄만
세로로 쌓이고 각 줄이 `space-between`이라 1440px에서 연도와 버튼 사이가 998px 벌어져 있었다.

**세로도 잰다.** 폰 세로에서 첫 그림이 화면 어디쯤에서 시작하는지가 이 서비스의 지표다 —
이게 무슨 서비스인지 5초에 말하는 것이 그림이기 때문이다.

| 375px 홈 | 첫 그림 시작 y (화면 740px 기준) |
|---|---|
| 상단 바 넣기 전 | 421px |
| 상단 바(44px)만 얹었을 때 | **522px** (71%) |
| 검색을 접고 중복된 줄을 지운 뒤 | 428px (58%) |
| 검색을 다시 펴고(+8px) 겹친 경계를 걷은 뒤 | **383px** (52%) |

1440px에서도 같이 줄었습니다 — 머리말 144→**98px**, 첫 그림 y 377→**326px**,
그림 위 **전폭 가로선 3개 → 2개**(상단 바 밑줄 + 해 제목의 시기 밑줄).

### 🔴 같은 375px에서 343px일 때도 있고 328px일 때도 있다 — 둘 다 맞다

**스크롤바가 레이아웃 폭을 먹느냐가 갈랐다.**

| 어디서 | `innerWidth` | `clientWidth` | 카드 | 왜 |
|---|---|---|---|---|
| **폰** (오버레이 스크롤바) | 375 | **375** | **343px** | `375 − 32`(`.page` 좌우 패딩 16+16) |
| **데스크탑** (자리를 차지하는 스크롤바) | 375 | **360** | **328px** | 스크롤바가 **15px**을 먼저 가져간다 |

위 표에 적은 **343이 주 사용자의 값**이다 — 폰 세로 한 손.

그런데 **재는 쪽은 데스크탑이다.** 창 크기 조절 도구를 못 믿게 된 뒤로
(그 도구가 `375` 요청에 성공을 반환하고 실제로는 500을 줬다 — 아래 [AI 도구 사용 내역](#ai-도구-사용-내역))
375px 레이아웃은 **폭 375px짜리 `<iframe>` 안에서** 잰다.
macOS 최소 창 너비 때문에 창으로는 375가 안 되기 때문이다.
**그 iframe이 데스크탑 스크롤바를 달고 있어서, 재면 328이 나온다.**

```js
// 폰 쪽 값을 재려면 iframe 안에서 스크롤바를 걷어야 한다
d.head.appendChild(Object.assign(d.createElement('style'), {
  textContent: 'html{scrollbar-width:none}html::-webkit-scrollbar{display:none}'
}));
// → clientWidth 360 → 375, 카드 328 → 343
```

> 🔑 **재는 방법이 값을 15px 바꾼다.** 어느 하나가 틀린 게 아니라 **둘이 다른 것을 재고 있었다.**
...
+ 한글 음절 전체(11,172자)로 서브셋                    1,701KB
+ KS X 1001 완성형(2,350자) + ASCII + 자모 + 문장부호    438KB
+ 가변축을 wght 400~700으로 제한                        309KB   ← 싣는 것
```

축 제한이 무손실인 근거: **이 앱이 쓰는 굵기는 400·500·600·700 넷뿐**입니다
(`globals.css` 전수 — 600이 15회, 500·400이 3회씩, 700이 2회). 쓰지 않는 굵기에 129KB를 얹지 않습니다.

**왜 시스템 글꼴로 두지 않았나** — macOS는 Apple SD Gothic Neo, Windows는 맑은 고딕이라
**보는 사람마다 다른 화면**이었습니다. 이 README는 *1440px에서 카드가 322px*이라고 적어왔는데
그 수가 **재는 사람의 OS에 달려 있었습니다.** 글꼴을 고정해야 그 값이 재현됩니다.

> #### 🔴 이 판단은 한 번 뒤집혔습니다. 접었던 근거가 틀렸습니다
>
> 처음에는 *"원본이 2,009KB"*라며 접고, 근거를 이렇게 적었습니다 —
> *"1.7MB(목록 쿼리의 사진 바이트)를 비싸다고 해놓고 2MB를 글꼴로 얹으면 앞뒤가 안 맞는다."*
>
> **1.7MB는 매 요청이고 글꼴은 한 번 받고 캐시됩니다.** 비교할 수 없는 둘을 나란히 놨습니다.
> 그리고 *"직접 서브셋하면 300KB지만 그 값이 대가보다 크지 않다"*고 덧붙였는데,
> **그건 측정이 아니라 회피였습니다.** 재보니 309KB이고, 이 저장소가 이미 싣고 있는
> 시드 이미지 합계(334KB)보다 작습니다.

**남는 한계**: KS X 1001 밖의 희귀 음절은 시스템 글꼴로 떨어집니다. 한 문장 안에서 글꼴이
섞일 수 있습니다. 2,350자는 현대 한국어 표기용 표준 집합이라 실제로 걸리는 일은 드물지만,
**안 걸린다고는 말할 수 없습니다.**
...
> 같은 README의 화면 표는 12행이고, 실제로는 `page.tsx` 12개 + `not-found` + `error` = **14화면**입니다.
> 확장하면서 화면을 셋 더 만들어 **표에는 넣고 이 줄은 안 봤습니다.**
>

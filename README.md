...
| **모아서 본다** | `모아보기` — 해마다 나뉜 사진 격자. 아이 말로 찾을 수 있습니다 |
...
→ <http://localhost:3000> 을 열면 **타임라인**이 뜨고, 임신 14주부터 초등 저학년까지 **12점이 이미 들어 있다.**
왼쪽 사이드바(폰이면 하단 탭)에 **타임라인 · 캘린더 · 모아보기 · 되짚어보기 · 책** 다섯 칸이 있다.
...
| 요구 | 어디서 |
|---|---|
| 뭘 하는 서비스인지 · 다음에 뭘 누를지 | 첫 화면(타임라인) 머리말 두 줄 + 사이드바 맨 위의 **화면에서 유일하게 진한 버튼** |
...
| **다른 화면 크기** | 900px을 경계로 **사이드바와 하단 탭이 교대**합니다. 격자는 폭에 따라 열 수가 저절로 늘어납니다. 아래 실측값 참조 |
| 망가뜨려도 됨 | **[처음 상태로 되돌리기]** |
...
| | 상태 | 왜 |
|---|---|---|
| **12점** | 등록되어 있음 | 켜자마자 이게 무슨 서비스인지 보인다. **임신 14주부터 초등 저학년까지** — 타임라인이 그 8년을 한 줄로 보여준다 |
...
**타임라인 맨 아래**에 있습니다. **되돌릴 수 없으면 아무도 안 눌러본다**고 보고 먼저 만들었습니다.
...
|---|---|---|
| **타임라인** | `/` | 8~9년을 **한 줄로** 보기. 부르는 단위가 언제 갈아탔는지 알기 |
| **캘린더** | `/calendar` | 그달 남긴 것과 **지금 열려 있는 검진 창**을 같이 보기 |
| **모아보기** | `/grid` | 사진을 격자로 훑기. **아이 말로 찾기** |
...
| 상세 | `/artwork/[id]` | 사진을 원본 대신 삼을 만큼 크게 보기 |
...
v1의 첫 화면은 **격자**였습니다. 지금 그 격자는 `/grid`에 있고, 첫 화면은 **타임라인**입니다.
...
> 🔴 **v1보다 칸이 작아졌습니다.** 1440px에서 322 → 268px, 375px에서 343 → 327px.
> 사이드바가 240px를 가져갔고 화면 좌우 여백이 16 → 24px로 늘었기 때문입니다.
> **줄어든 것이 아니라 자리를 내준 것**이고, 그 대가로 다섯 화면이 어디서나 한 번에 닿습니다.
